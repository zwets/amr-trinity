rule get_rgi_bwt_db:
    output:
        card_db_bwt = os.path.join(config['db_dir'], "card_bwt", "card.json")
    params:
        db_dir = os.path.join(config['db_dir'], "card_bwt")
    log:
        "logs/rgi_bwt_db.log"
    shell:
        """{{
...
rule run_rgi_bwt:
    input:
        read1 = get_read1,
        read2 = get_read2,
        card_db = os.path.join(config['db_dir'], "card_bwt", "card.json")
    output:
